// examples/gpu_euroc_live.rs
//
// Live visualization of the Rudolf-V GPU frontend on EuRoC data.
//
// Identical to euroc_live.rs except it uses GpuFrontend instead of Frontend.
// GPU stages: pyramid build, FAST detection, KLT tracking.
// CPU stages: histogram equalization, NMS, occupancy grid, RANSAC.
//
// Usage:
//   cargo run --example gpu_euroc_live --release -- /path/to/MH_01_easy
//   cargo run --example gpu_euroc_live --release -- /path/to/MH_01_easy 500
//
// Controls:
//   Space  — pause/resume
...
//   H      — cycle histogram equalization (none → global → CLAHE)
...
fn main() {
    ...
    let camera = match CameraIntrinsics::from_euroc_yaml(&sensor_yaml) {
        ...
        Err(e) => {
            eprintln!("Warning: no sensor.yaml ({e}), geometric verification disabled");
            None
        }
    };
    ...
    while window.is_open() && !window.is_key_down(Key::Escape) && !window.is_key_down(Key::Q) {
            ...
            || window.is_key_pressed(Key::NumPadPlus, minifb::KeyRepeat::No)
        {
            ...
            println!("Delay: {frame_delay_ms}ms");
        }
        ...
        if should_advance {
            ...
            let (features, stats) = frontend.process(&gpu, &img);
...
// ---------------------------------------------------------------------------
// EuRoC I/O (identical to euroc_live.rs)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Framebuffer rendering (identical to euroc_live.rs)
// ---------------------------------------------------------------------------
...
fn render_grayscale(img: &Image<u8>, fb: &mut [u32], img_w: usize, img_h: usize, scale: usize) {
    for y in 0..img_h {
        for x in 0..img_w {
            ...
            let pixel = (v << 16) | (v << 8) | v;
            for sy in 0..scale {
                for sx in 0..scale {
                    fb[(y * scale + sy) * (img_w * scale) + x * scale + sx] = pixel;
                }
            }
        }
    }
...
fn draw_arrow(fb: &mut [u32], w: usize, h: usize, x0: i32, y0: i32, x1: i32, y1: i32, color: u32) {
    ...
    let ay = y1 as f32 - head * (uy - 0.4 * ux);
    let bx = x1 as f32 - head * (ux - 0.4 * uy);
    let by = y1 as f32 - head * (uy + 0.4 * ux);
